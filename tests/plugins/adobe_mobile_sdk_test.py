...
#
import pytest
from truegaze.plugins import AdobeMobileSdkPlugin


# Tests for AdobeMobileSdkPlugin
class TestBasePlugin(object):
    def test_plugin_properties_name_desc(self):
        plugin = AdobeMobileSdkPlugin()
        assert not plugin.name.startswith('Base')
        assert not plugin.desc.startswith('Base')

    def test_plugin_properties_support(self):
        plugin = AdobeMobileSdkPlugin()
        assert plugin.supports_android is True
        assert plugin.supports_ios is True

    def test_check_support_is(self):
        plugin = AdobeMobileSdkPlugin()
        assert plugin.check_supported_os(False, False) is False
        assert plugin.check_supported_os(False, True) is True
        assert plugin.check_supported_os(True, False) is True
        assert plugin.check_supported_os(True, True) is True
